...
import '../../../data/models/package/specific_package_name.dart';
...
import '../package/cubit/states.dart';
import 'booking_packages.dart';
import 'package:flutter_bloc/src/bloc_consumer.dart';
import 'package:flutter_bloc/src/bloc_provider.dart';
class DetailsPackagePageForAllPackages extends StatefulWidget {
  const DetailsPackagePageForAllPackages({
    Key? key,
  ...
  @override
  State<DetailsPackagePageForAllPackages> createState() => _DetailsPackagePageForAllPackagesState();
}

class _DetailsPackagePageForAllPackagesState extends State<DetailsPackagePageForAllPackages> {
  int sumPrice = 0;
  ...
  SpecificPackageModel? specificPackageModel;

  @override
  Widget build(BuildContext context) {
    print("widget.stableId:${widget.stableId}");
    return BlocProvider(
      create: (context) => AllPackageCubitPage()..getSpecificPackage(packageId: widget.packageId),
      child: BlocConsumer<AllPackageCubitPage, PackageCubitPageStates>(
        listener: (BuildContext context, state) {
          if (state is GetSpecificPackageSuccessStates) {
            print(state.specificPackageModel.message);
            specificPackageModel = state.specificPackageModel;
        ...
        builder: (BuildContext context, state) {
          ...
          print(sumPrice);
    return Scaffold(
      body: state is GetSpecificPackageLoadingStates ? Center(
        ...
        children: [
          Container(
              ...
              child: Stack(children: [
                specificPackageModel?.data?.image == '' ?Container():  Container(
                  child: Image.network(
                    specificPackageModel?.data?.image.toString() ?? "",
                          ...
                          child: SingleChildScrollView(
                            child: Column(
                                ...
                                children: [
                                  ...
                                  text(
                                      title: specificPackageModel?.data?.name ??
                                          '',
                                  ...
                                  text(
                                      title: specificPackageModel?.data?.description??
                                          '',
                                      color: Colors.black,
                                      fontSize: 14,
                                      ...
                                      Container(
                                        child: Row(children: [
                                          text(
                                              ...
                                              fontWeight: FontWeight.w400),
                                          text(
                                              title: '${specificPackageModel?.data?.price ?? 0} AED',
                                              color:
                                  ...
                                  Container(
                                    ...
                                    child: ListView.builder(
                                        itemCount: specificPackageModel?.data?.services.length,
                                                ...
                                                children: [
                                                  Row(
                                                    children: [
                                                      ...
                                                      text(
                                                          title: specificPackageModel?.data?.services[
                                                                      index]
                                                                  .name ??
                                                              'undefined',
